...
M.is_inside_tag = function()
    ...
    if closing_tag_start then
        ...
        if first_close then
            ...
            local closing_tag_content = before:sub(closing_tag_start + 1)
                .. after:sub(1, first_close - 1)
    ...
    if opening_tag_start then
        ...
        if first_close then
            ...
            if not has_close_between then
                ...
                local tag_content = before:sub(opening_tag_start + 1)
                    .. after:sub(1, first_close - 1)
...
M.calculate_indentation = function()
    ...
    local inner_indent = indent
        .. (use_tabs and "\t" or string.rep(" ", tab_size))
